// @insp https://www.shadertoy.com/view/3scfD7
//	34 FPS c включ оптим шейдеров, не нравятся возникающие артефакты от отражения неба с черной оконтовкой
#include "stdafx.h"
...
template<> class jarble<DxVer::v9> : public CurClientApp<DxVer::v9> { using T = DxVer::v9;
	...
	bool init(DxCtx<T>::cref_ptr_t crpustDxCtx, ToolCtx<T>::cref_ptr_t puoTools, Adjust<T>* poAdjustDxAux) {
		...
		m_pcVs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Vs( 
				[]() ->const auto & {
					static 
#include "resource\jarble_vs_Dx9.hlsl.h"
					return g_main;
				}
			);
		m_pcPs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Ps( 
				[]() ->const auto & {
					static 
#include "resource\jarble_ps_Dx9.hlsl.h"
					return g_main;
				}
			);
		if ( !m_pcVs || !m_pcPs )
		...
		bool bRv = m_puPsConstSetter ->setFloat4( "iResolution", D3DXVECTOR4{ (float)stPp.BackBufferWidth, (float)stPp.BackBufferHeight, 0, 0 } );
	...
	void render_frame(DxCtx<T>::cref_ptr_t crpustDxCtx, Dynamic<T>::cref_ptr_t) {
        Sys::Hr hr;
		m_puPsConstSetter ->setFloat( "iTime", m_oTimer.get( ) );
...
template<> class jarble<DxVer::v10> : public CurClientApp<DxVer::v10> { using T = DxVer::v10;
	...
	bool init(DxCtx<T>::cref_ptr_t crpustDxCtx, ToolCtx<T>::cref_ptr_t puoTools, Adjust<T>* poAdjustDxAux) {
		...
		m_pcVs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Vs( 
				[]() ->const auto & {
					static 
#include "resource\jarble_vs_Dx10.hlsl.h"
					return g_main;
				}
				, &veShaderByte
			);
		m_pcPs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Ps( 
				[]() ->const auto & {
					static 
#include "resource\jarble_ps_Dx10.hlsl.h"
					return g_main;
				}
			);
		if ( !m_pcVs || !m_pcPs )
...
template<> class jarble<DxVer::v11> : public CurClientApp<DxVer::v11> { using T = DxVer::v11;
	...
	bool init(DxCtx<T>::cref_ptr_t crpustDxCtx, ToolCtx<T>::cref_ptr_t puoTools, Adjust<T>* poAdjustDxAux) {
		...
		m_pcVs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Vs( 
				[]() ->const auto & {
					static 
#include "resource\jarble_vs_Dx11.hlsl.h"
					return g_main;
				}
				, &veShaderByte
			);
		m_pcPs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Ps( 
				[]() ->const auto & {
					static 
#include "resource\jarble_ps_Dx11.hlsl.h"
					return g_main;
				}
			);
		if ( !m_pcVs || !m_pcPs )
...
template<> class jarble<DxVer::v12> : public CurClientApp<DxVer::v12> { using T = DxVer::v12;
	LikeDxut::Timer m_oTimer;
	...
	bool init(DxCtx<T>::cref_ptr_t crpustDxCtx, ToolCtx<T>::cref_ptr_t puoTools, Adjust<T>* poAdjustDxAux) {
		...
		m_pcVs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Vs( 
				[]() ->const auto & {
					static 
#include "resource\jarble_vs_Dx12.hlsl.h"
					return g_main;
				}
			);
		m_pcPs = puoTools ->shader( ) ->loader( ) ->arrayC( ) ->fromHeader( ) ->Ps( 
				[]() ->const auto & {
					static 
#include "resource\jarble_ps_Dx12.hlsl.h"
					return g_main;
				}
			);
		if ( !m_pcVs || !m_pcPs )
...
uptr< Sys::HolderClientApp > ClientTy::entryPoint() { 
	return std::make_unique< Sys::HolderClientApp >(new jarble<CurDxVer>()); }
